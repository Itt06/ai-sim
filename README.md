...
このリポジトリは TownBox の日本語化版です。ゲームタイトル TownBox は固有名詞として残し、画面のメニュー、操作説明、都市情報、住民情報、職業、サービス、行動・イベント表示を日本語化しています。
...
npm ci
npm run package
npx browser-sync start --server ./bin
```

ブラウザで http://localhost:3000 を開いてください。

履歴アセットはサイズが大きいためリポジトリには含めていません。アセットがない場合はゲーム内のフォールバック生成が使用されます。
