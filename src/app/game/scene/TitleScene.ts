...
export default class TitleScene extends Phaser.Scene {
    ...
    create(): void {
        ...
        // Subtitle
        const subtitleText = this.add.text(cx, cy - 22, '街づくりシミュレーション', {
            fontSize: '18px',
            fontFamily: 'system-ui, sans-serif',
            color: '#9ecf92',
        }).setOrigin(0.5).setAlpha(0);
        ...
        // Button label
        const btnText = this.add.text(cx, btnY, 'ゲームを始める', {
            fontSize: '22px',
            fontFamily: 'system-ui, sans-serif',
            fontStyle: 'bold',
            color: '#e8f5e9',
        ...
        const loadText = this.add.text(cx, loadBtnY, 'ゲームを読み込む', {
            fontSize: '18px',
            fontFamily: 'system-ui, sans-serif',
            fontStyle: 'bold',
            color: '#bfe0c4',
    ...
    private async handleLoadClick(loadText: Phaser.GameObjects.Text): Promise<void> {
        ...
        if (!hasSave) {
            loadText.setText('セーブデータがありません');
            this.time.delayedCall(1500, () => loadText.setText('ゲームを読み込む'));
            return;
        }
