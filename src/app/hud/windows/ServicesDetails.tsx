...
const ServicesDetails: FC<DetailsWindowProps> = ({ game, index, data, z, onFocus, onClose }) => {
    ...
    return (
        <Window game={game} index={index} z={z} onFocus={onFocus} title="都市サービス" testId="window-services" initialSize={INITIAL_SIZE} onClose={onClose}>
            <div style={{ padding: '4px 10px' }} data-testid="services-rows">
                {lines.length === 0 && <p>都市サービスの充足状況はまだ計測されていません。</p>}
                {lines.map(line => {
                    ...
                    return (
                        <section key={line.service} data-testid={`service-row-${line.service}`} style={{ marginBottom: 10 }}>
                            <h4 style={{ margin: '4px 0', color: critical ? '#c0392b' : undefined }}>
                                {ja(line.label)}: {(line.ratio * 100).toFixed(0)}%
                            </h4>
                            <div style={{ fontSize: 12 }}>
                                {line.facilities === 0
                                    ? '施設なし'
                                    : `提供者 ${line.providers}人・施設 ${line.facilities}か所・必要数 ${line.needed}`}
                                {critical && SERVICES.services[line.service] && (
                                    <div style={{ color: '#c0392b', marginTop: 2 }}>{ja(SERVICES.services[line.service]!.warning)}</div>
                                )}
                            </div>
